...
@synthesize tag, hudView, maskType, showNetworkIndicator, fadeOutTimer, stringLabel, imageView, spinnerView, previousKeyWindow, visibleKeyboardHeight;
...
{
    [[SVProgressHUD sharedView] setTag:tag];
    [SVProgressHUD showWithStatus:nil networkIndicator:SVProgressHUDShowNetworkIndicator];
}
...
+ (void)dismisswWithTag:(int)tag
{
    if ([[SVProgressHUD sharedView] tag] == tag) {
        [[SVProgressHUD sharedView] dismiss];
    }
}
